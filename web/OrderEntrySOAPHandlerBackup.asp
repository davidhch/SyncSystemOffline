...
'preform db lookup for any items that are not in sync
	...
	nCreditLimit = objxmldom.selectSingleNode("SOAP:Envelope/SOAP:Body/Request/MTA").Text

	If strFunctionCall = "SendCustomerDetailsXML" Then
		Call SendCustomerDetailsXML(nResellerID,nCustomerID,nOrderEntryCID,strCustomerName,strCompany,strAddress1,strAddress2,strCity,strCounty,strPostCode,strCountry,strEmailAddress,strTelNo,strNewsletterTitle,strNewsletterEmail,dLastDate,nTotalNewsSent,bActive,nTimesLoggedIn,bDemo,strHashKey,strMTA,nCreditLimit)
	End If
	
	Function SendCustomerDetailsXML(nResellerID,nCustomerID,nOrderEntryCID,strCustomerName,strCompany,strAddress1,strAddress2,strCity,strCounty,strPostCode,strCountry,strEmailAddress,strTelNo,strNewsletterTitle,strNewsletterEmail,dLastDate,nTotalNewsSent,bActive,nTimesLoggedIn,bDemo,strHashKey,strMTA,nCreditLimit)
		
		nTableID = 6
		nOfflineID = nOrderEntryCID
		nOnlineID = nCustomerID	
		bLocked = 1
		bExecuted = 0
	...
	'Response.Write "<br>YES I AM BLOODY HERE!!!"
	
	If strFunctionCall = "SendOrderXML" Then
		Call SendOrderXML(nCustomerID,nOrderEntryCID,nQuotationID,nEmployeeID,dOrderDate,strPurchaseOrderNumber,strInvoiceDescription,strShipName,strShipAddress,strShipCity,strShipStateOrProvince,strShipPostalCode,strShipCountry,strShipPhoneNumber,dShipDate,nShippingMethodID,nFreightCharge,nSalesTaxRate,nLeadReferralID,strProposalReference,dEntryDate,bCommissionPaid,nInvoiceID)
	End If
	
	Function SendOrderXML(nCustomerID,nOrderEntryCID,nQuotationID,nEmployeeID,dOrderDate,strPurchaseOrderNumber,strInvoiceDescription,strShipName,strShipAddress,strShipCity,strShipStateOrProvince,strShipPostalCode,strShipCountry,strShipPhoneNumber,dShipDate,nShippingMethodID,nFreightCharge,nSalesTaxRate,nLeadReferralID,strProposalReference,dEntryDate,bCommissionPaid,nInvoiceID)
	...
	If strFunctionCall = "SendOrderDetailsXML" Then
		Call SendOrderDetailsXML(nID,nOrderID,nNewZappOrderID,nProductID,nNewZappProductID,nQuantity,nUnitPrice,nDiscount,nAccountType,nOrderEntryCID,nCID,dRenewalDate,bStatistics,bFrontPage,bServerSideIncludes,bSSL,bIndexServer,bCurrent)
	End If

	Function SendOrderDetailsXML(nID,nOrderID,nNewZappOrderID,nProductID,nNewZappProductID,nQuantity,nUnitPrice,nDiscount,nAccountType,nOrderEntryCID,nCID,dRenewalDate,bStatistics,bFrontPage,bServerSideIncludes,bSSL,bIndexServer,bCurrent)
	...
	Function SendPaymentXML(nPaymentsID,nOrderEntryPaymentsID,nNewZappOrderID,nOnlinePaymentID,nOrderID,nPaymentAmount,dPaymentDate,nCreditCardID,nPaymentMethodID,strTransactionCode,strProcessingValid,strProcessingCode,strProcessingCodeMessage,strAuthCode,strAuthMessage,strResponseCode,strResponseCodeMessage,nAmountToCharge)
		...
		nTableID = 3
		nOfflineID = nOrderEntryPaymentsID
		nOnlineID = nPaymentsID	
		bLocked = 1
		bExecuted = 0
