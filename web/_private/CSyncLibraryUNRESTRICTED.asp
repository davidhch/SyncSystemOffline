...
Class CSyncLibrary
	...
	Public Sub CompleteSyncProcess(nOnlineID,nOfflineID,nTableID,bLocked,bExecuted)
		...
		logSoapDebug("XML:"&soapXML)
		logSoapDebug("URL:"&soapURL)
	...
	Public Function saveProduct(strProductName,nProductCategoryID,nUnitPrice,nUnitCost,strOneOffCost,nSupplierID,strDescription,strSystemDescription,nSetupPrice,nOrder,bActive)
		...
		' ideally we'd like a unique identifier 
	...
	Sub GetAccountType(nProductID,nAccountType)
	
			objDBInterface.NewSQL
			objDBInterface.AddFieldToQuery HOSTINGACCOUNTTYPES_ID
			objDBInterface.AddWhere HOSTINGACCOUNTTYPES_PRODUCTID, "=", nProductID, "number", "and"
			objDBInterface.QueryDatabase
			
			nAccountType = objDBInterface.GetValue(HOSTINGACCOUNTTYPES_ID)
	
	End Sub
	
	Sub GetWebHosting(nNewZappCID,nWebHostingID)
			...
			nWebHostingID = objDBInterface.GetValue(WEBHOSTING_ID)
	End Sub
	
	Sub GetPOPAccountID(nWebHostingID,nPOPAccountID)
			
			Response.Write "nWebHostingID " & nWebHostingID
	
			objDBInterface.NewSQL
			objDBInterface.AddFieldToQuery POPACCOUNTS_ID
			objDBInterface.AddWhere POPACCOUNTS_WEBHOSTINGID, "=", nWebHostingID, "number", "and"
			objDBInterface.QueryDatabase
			
			nPOPAccountID = objDBInterface.GetValue(POPACCOUNTS_ID)
			
	End Sub
	
	Public Function saveAccountData(nNewZappCID,nOrderEntryCID,nWebHostingID,dRenewalDate,strUserName,strPassword,bActiveStatus,bDemoStatus,bUsedStatus,nCustomerProductID,nProductID,nYears)
			
		bActiveStatus = covertBooleanTextToBool(bActiveStatus)
		bDemoStatus = covertBooleanTextToBool(bDemoStatus)
		
		If Len(nProductID) > 0 Then 
		    GetAccountType nProductID,nAccountType
		End If
		
		saveAccountData = getAutoGeneratedID(nNewZappCID,WEBHOSTING_ID,WEBHOSTING_CID)
		
		If Len(saveAccountData) > 0 Then
		
			updateAccountData saveAccountData,nNewZappCID,nOrderEntryCID,nWebHostingID,dRenewalDate,strUserName,strPassword,bActiveStatus,bDemoStatus,bUsedStatus,nCustomerProductID,nProductID,nYears
		
		Else
			
			objDBInterface.NewSQL         
			objDBInterface.StoreValue WEBHOSTING_CUSTOMERID, nOrderEntryCID
			objDBInterface.StoreValue WEBHOSTING_CID, nNewZappCID 
			objDBInterface.StoreValue WEBHOSTING_RENEWALDATE, dRenewalDate
			objDBInterface.StoreValue WEBHOSTING_ACCOUNTTYPE, nAccountType
			objDBInterface.StoreValue WEBHOSTING_CURRENT, bActiveStatus
			objDBInterface.StoreValue WEBHOSTING_NOYEARSHOSTING, nYears
			objDBInterface.StoreValue WEBHOSTING_DEMOMODE, bDemoStatus
			objDBInterface.InsertUsingStoredValues "Web Hosting"
		
			'need to get this in order to link the records
			GetWebHosting nNewZappCID,nWebHostingID
			...
			objSavePOPAccounts.StoreValue POPACCOUNTS_SITEADMIN, 1
			objSavePOPAccounts.StoreValue POPACCOUNTS_PRIMARYACCOUNT, 1
			objSavePOPAccounts.StoreValue POPACCOUNTS_SHELLACCESS, 0
			objSavePOPAccounts.StoreValue POPACCOUNTS_USERFPX, 0
	...
	Public Sub updateAccountData(nOfflineID,nNewZappCID,nOrderEntryCID,nWebHostingID,dRenewalDate,strUserName,strPassword,bActiveStatus,bDemoStatus,bUsedStatus,nCustomerProductID,nProductID,nYears)
		
		bActiveStatus = covertBooleanTextToBool(bActiveStatus)
		bDemoStatus = covertBooleanTextToBool(bDemoStatus)
		
		LogOrder -1, -1, "CID:"&nNewZappCID&" - RenewalDate:"&dRenewalDate&"", 5
        
        If Len(nProductID) > 0 Then 
		    GetAccountType nProductID,nAccountType
		End If
		
		'UpdateSingleRecordDB nOrderEntryCID, nOfflineID, "Web Hosting", WEBHOSTING_CUSTOMERID, WEBHOSTING_ID
		UpdateSingleRecordDB nNewZappCID, nOfflineID, "Web Hosting", WEBHOSTING_CID, WEBHOSTING_ID
		UpdateSingleRecordDB dRenewalDate, nOfflineID, "Web Hosting", WEBHOSTING_RENEWALDATE, WEBHOSTING_ID
		UpdateSingleRecordDB nAccountType, nOfflineID, "Web Hosting", WEBHOSTING_ACCOUNTTYPE, WEBHOSTING_ID
		UpdateSingleRecordDB bActiveStatus, nOfflineID, "Web Hosting", WEBHOSTING_CURRENT, WEBHOSTING_ID
		UpdateSingleRecordDB nYears, nOfflineID, "Web Hosting", WEBHOSTING_NOYEARSHOSTING, WEBHOSTING_ID
		UpdateSingleRecordDB bDemoStatus, nOfflineID, "Web Hosting", WEBHOSTING_DEMOMODE, WEBHOSTING_ID

		GetPOPAccountID nOfflineID,nPOPAccountID
		
		UpdateSingleRecordDB nWebHostingID, nPOPAccountID, "POP Accounts", POPACCOUNTS_WEBHOSTINGID, POPACCOUNTS_ID
		UpdateSingleRecordDB strUserName, nPOPAccountID, "POP Accounts", POPACCOUNTS_USERNAME, POPACCOUNTS_ID
		UpdateSingleRecordDB strPassword, nPOPAccountID, "POP Accounts", POPACCOUNTS_PASSWORD, POPACCOUNTS_ID
		
		nWebHostingID = nOfflineID
		
		UpdateSingleRecordDB nWebHostingID, nOfflineID, "HostingAccountTypes", HOSTINGACCOUNTTYPES_PRODUCTID, HOSTINGACCOUNTTYPES_ID
	...
	Public Sub processSoap()
		...
		Select Case objSoap.ReadSingleNode("FunctionCall")
			...
			Case "SendPaymentXML"
				...
				'soapURL = strSoapFolder & "AddPaymentSOAPOO_Additions.asp"
			...
			Case "SendAccountDetailsXML"
				nOnlineID	= objSoap.ReadSingleNode ("NewZappCID")
				nOfflineID 	= objSoap.ReadSingleNode ("WebHostingID")
				nTableID 	= 9
				
				soapURL = strSoapFolder & "AddAccountDetailsSOAPOO.asp"
				objSoap.clearSendNodes()
				addNodeWithSameTagAsNodeRead "WebHostingID"
				addNodeWithSameTagAsNodeRead "NewZappCID"
				addNodeWithSameTagAsNodeRead "OrderEntryCID"
				addNodeWithSameTagAsNodeRead "RenewalDate"
				addNodeWithSameTagAsNodeRead "Active"
				addNodeWithSameTagAsNodeRead "Demo"
				addNodeWithSameTagAsNodeRead "UserName"
				addNodeWithSameTagAsNodeRead "Password"
				addNodeWithSameTagAsNodeRead "ProductID"
				addNodeWithSameTagAsNodeRead "NumberOfYears"
	...
	Public Sub syncCheck()
		...
		If nRecords > 0 Then
			logSoapDebug("******************We have to sync the records")
	...
	'response.write "SyncData: <br>"
		Select Case nTableID
			...
			Case 9
				...
				'logSoapDebug("******************We are in the correct case")
			
				objDBInterface.NewSQL
				objDBInterface.AddFieldToQuery WEBHOSTING_ID
				objDBInterface.AddFieldToQuery WEBHOSTING_CUSTOMERID
				objDBInterface.AddFieldToQuery WEBHOSTING_CID
				objDBInterface.AddFieldToQuery WEBHOSTING_ACCOUNTTYPE
				objDBInterface.AddFieldToQuery WEBHOSTING_NOYEARSHOSTING
				objDBInterface.AddFieldToQuery WEBHOSTING_RENEWALDATE
				objDBInterface.AddFieldToQuery WEBHOSTING_CURRENT
				objDBInterface.AddFieldToQuery WEBHOSTING_DEMOMODE
				objDBInterface.AddWhere WEBHOSTING_ID, "=", nOfflineID, "number", "and"
				objDBInterface.QueryDatabase
				
				'logSoapDebug("******************Query Number 1 Start")
				
				nWebHostingID = objDBInterface.GetValue(WEBHOSTING_ID)
				nOrderEntryCID = objDBInterface.GetValue(WEBHOSTING_CUSTOMERID)
				nNewZappCID = objDBInterface.GetValue(WEBHOSTING_CID)
				nProductID = objDBInterface.GetValue(WEBHOSTING_ACCOUNTTYPE)
				nNumberOfYears = objDBInterface.GetValue(WEBHOSTING_NOYEARSHOSTING)
				dRenewalDate = objDBInterface.GetValue(WEBHOSTING_RENEWALDATE)
				bActive =  objDBInterface.GetValue(WEBHOSTING_CURRENT)
				bDemo = objDBInterface.GetValue(WEBHOSTING_DEMOMODE)
				
				'logSoapDebug("******************Query Number 1 Complete")
				
				'logSoapDebug("******************nOfflineID ") & nOfflineID
				
				Set objSyncPOPAccounts = Nothing
				Set objSyncPOPAccounts = New CDBInterface
				objSyncPOPAccounts.Init arrCustDetails, nTotCustDetails	
				
				objSyncPOPAccounts.NewSQL
				objSyncPOPAccounts.AddFieldToQuery POPACCOUNTS_WEBHOSTINGID
				objSyncPOPAccounts.AddFieldToQuery POPACCOUNTS_USERNAME
				objSyncPOPAccounts.AddFieldToQuery POPACCOUNTS_PASSWORD
				objSyncPOPAccounts.AddWhere POPACCOUNTS_WEBHOSTINGID, "=", nOfflineID, "number", "and"
				objSyncPOPAccounts.QueryDatabase
				
				'logSoapDebug("******************Query Number 2")
				
				strUserName = objSyncPOPAccounts.GetValue(POPACCOUNTS_USERNAME)
				strPassword = objSyncPOPAccounts.GetValue(POPACCOUNTS_PASSWORD)
				
				objSoap.clearSendNodes()
				objSoap.AddSendNode "FunctionCall", "SendAccountDetailsXML"
				objSoap.AddSendNode "WebHostingID", nWebHostingID
				objSoap.AddSendNode "NewZappCID", nNewZappCID
				objSoap.AddSendNode "OrderEntryCID", nOrderEntryCID
				objSoap.AddSendNode "RenewalDate", dRenewalDate
				objSoap.AddSendNode "Active", bActive
				objSoap.AddSendNode "Demo", bDemo
				objSoap.AddSendNode "UserName", strUserName
				objSoap.AddSendNode "Password", strPassword
				objSoap.AddSendNode "ProductID", nProductID
				objSoap.AddSendNode "NumberOfYears", nNumberOfYears
				
				'logSoapDebug("******************All queries complete why are you not unlocking????")

			Case Else
				' do nothing
		End Select
		'response.write "<br>SOAP: " & objSoap.buildSendSoap
		sendSoapToUrl "http://orderentry-soap.destinet.co.uk/OrderEntrySOAPHandlerUNRESTRICTED.asp" 
	...
	Function AllowCID(nOfflineID,nTableID)
		...
		'If nOrderEntryCustomerID > 1379 Then
			AllowCID = True
		'End If
	...
	Function covertBooleanTextToBool(strBooleanText)
	
		covertBooleanTextToBool = strBooleanText
		
		If lcase(strBooleanText) = "true" Then
				covertBooleanTextToBool = 1
		End If
			
		If lcase(strBooleanText) = "false" Then
				covertBooleanTextToBool = 0
		End If
