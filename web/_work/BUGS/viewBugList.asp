...
<table  border="0" cellspacing="0" cellpadding="0">
  <tr style="position:relative;  top:expression(this.offsetParent.scrollTop); ">
	...
	<div style="float:right;">BUG LIST</div>
	<a href="default.asp">HOME</a> | 
	
	<a href="viewBugList.asp">show all</a> | 
	
	<a href="viewBugList.asp?ShowComplete=1">show completed</a> | 
	
	<a href="viewBugList.asp?ShowComplete=0">show NOT completed</a>	</th>
  </tr>
  <tr style="position:relative;  top:expression(this.offsetParent.scrollTop); ">
...
set conn = Server.CreateObject("ADODB.Connection")
	...
	'rs.Open "Select * from bugs WHERE bugType = 3 ORDER BY bugPriority, bugSection, bugID ASC", conn
	If Len(bShowComplete)> 0 Then
		If bShowComplete = 1 Then
			rs.Open "Select * from bugs WHERE bugType = 3 AND bugComplete = true ORDER BY bugSection, bugPriority, bugID ASC", conn
		End If
		If bShowComplete = 0 Then
			rs.Open "Select * from bugs WHERE bugType = 3 AND bugComplete <> true ORDER BY bugSection, bugPriority, bugID ASC", conn
		End If
	Else
		rs.Open "Select * from bugs WHERE bugType = 3 ORDER BY bugSection, bugPriority, bugID ASC", conn
	End If
