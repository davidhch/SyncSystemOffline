...
<xsl:template match="/">
<xsl:param name="totalCountSubscribers" select="count(/Export/View_ReportSummary)"/>
...
<xsl:call-template name="for.loop">
	 ...
	 <xsl:with-param name="count"><xsl:value-of select="$totalCountSubscribers" /></xsl:with-param>
</xsl:call-template>
...
<xsl:for-each select="Export/View_ReportSummary">
  ...
  <!--//DATA STARTS HERE//-->
	...
	<xsl:for-each select="View_CUSTOMFIELDS">
...
<!--// BOF DC created xsl for loop //-->
<!--//
This will loop through all the View_ReportSummary nodes
When a row is found with data it will write that data to screen
and no longer execute the loop.
//-->
<xsl:template name="for.loop">
   ...
   <xsl:if test="$i &lt;= $count">
	  	...
	  	<xsl:for-each select="Export/View_ReportSummary[($i+0)]/View_CUSTOMFIELDS">
			<xsl:if test="string-length(Custom_Field_Name) != 0">
				<xsl:text>,</xsl:text>
				<xsl:value-of select="translate(string(Custom_Field_Name),',',';')" />
			</xsl:if>
		</xsl:for-each>
		
		<xsl:if test="string-length(Export/View_ReportSummary[($i+0)]/View_CUSTOMFIELDS/Custom_Field_Name) = 0">
		  <xsl:call-template name="for.loop">
