...
<xsl:for-each select="Export/View_ReportSummary/View_CUSTOMFIELDS">
	<xsl:text>,</xsl:text>
...
<xsl:call-template name="for.loop">
	 ...
	 <xsl:with-param name="bNodesFound">8</xsl:with-param>
</xsl:call-template>
...
<xsl:template name="for.loop">
   ...
   <xsl:param name="bNodesFound"  />
   
   <!--begin_: Line_by_Line_Output -->
   <xsl:if test="$i &lt;= $count">
      <br /> <b><xsl:value-of select="$i" />.</b>Hello world!<xsl:value-of select="$bNodesFound" />
   </xsl:if>

   <!--begin_: RepeatTheLoopUntilFinished-->
   <xsl:if test="$bNodesFound = 8">
      <xsl:call-template name="for.loop">
		  ...
		  <xsl:with-param name="bNodesFound">
