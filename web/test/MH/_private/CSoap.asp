...
Class CSoap
	...
	Public Sub clearSendNodes()
		strSendNodes = ""
	End Sub
	
	Public Sub AddOpenNode (strNodeName)
		strSendNodes = strSendNodes & "<"&strNodeName&">"
	End Sub
	
	Public Sub AddCloseNode (strNodeName)
		strSendNodes = strSendNodes & "</"&strNodeName&">"
	End Sub
	
	Public Sub AddSendNode (strNodeName, strNodeValue)
		strSendNodes = strSendNodes & "<"&strNodeName&">"&strNodeValue&"</"&strNodeName&">"
	End Sub
	...
	Public Sub SetEnvelopePath(strPath)
		strEnvelopePath = strPath
	End Sub
	...
	Public Function GetNodeList(strNodeName)
		...
		strNoOfChild = objNodeList.length		
		
		for i = 0 to (strNoOfChild - 1)
			set objchild = objNodeList.item(i)	
			objNoOfChild = objChild.childNodes.length
			
			for j = 0 to (objNoOfChild - 1)														 
				GetNodelist = GetNodelist  & objchild.childNodes(j).text & " - "					
			next	
	...
	Public Function GetNodeListLength()
		GetNodeListLength = objNodeList.length
	End Function
	
	Public Function GetNodeListText(strNodeName)
		Call SetNodeList (strNodeName) 
		For Each Elem In objNodeList 
   			Elem.text ' this is the value of the element
		Next
	End Function 
	
	Public Function getItemText(i)
		getItemText = objNodeList.item(i).text
	End Function
	...
	Public Function ProcessSoapRequest()
		...
		If bVerified = True Then		
			nRequestType = objSoap.ReadSingleNode("RequestType") ' Numeric or Named ???
	...
	Function buildSendSoap()
		...
		buildSendSoap = buildSendSoap & strSendNodes
		buildSendSoap = buildSendSoap & "</Request>"
		buildSendSoap = buildSendSoap & "</SOAP:Body>"
	...
	Public Function CheckLogin
		...
		If CheckString(objSoap.ReadSingleNode("CID")) AND CheckString(objSoap.ReadSingleNode("UserName")) AND CheckString(objSoap.ReadSingleNode("Password")) Then
			CheckLogin = lib_LoginCheck(objSoap.ReadSingleNode("CID"), objSoap.ReadSingleNode("UserName"),objSoap.ReadSingleNode("Password")) ' ??? use of a lib_ file within class
	...
	Public Function SendSoap(strXML, SoapServerURL)
		...
		SendSoap = objxmlhttp.Status
	
	End Function
	
	Public Function getResponseText()
		getResponseText = objxmlhttp.responseText
	End Function
	
	Public Function getResponseXML()
		getResponseXML = objxmlhttp.responseXML
	End Function
	
	Public Function getXML()
		getXML = objxmldom.xml
	End Function
	
	'***************************************************************
	'	FUNCTIONS LIBRARY - IT WONT BE HERE - THIS IS AN EXAMPLE
	'
